...
import { buildRawMessage, sendWithGmailApi } from "../scripts/gmail-api.mjs";
...
export async function processEmailOutbox(limit = 10) {
  ...
  for (let processed = 0; processed < limit; processed += 1) {
    ...
    const rows = await sql.query(
      `SELECT o.id, o.kind, o.attempts, s.email, s.status AS subscriber_status,
    ...
    try {
      ...
      await sql.query(
        "UPDATE email_outbox SET status = 'sent', sent_at = $1, gmail_message_id = $2, last_error = NULL, worker_id = NULL, locked_at = NULL WHERE id = $3 AND worker_id = $4",
        [now(), gmailMessageId, row.id, workerId]
      );
      summary.sent += 1;
    } catch (error) {
