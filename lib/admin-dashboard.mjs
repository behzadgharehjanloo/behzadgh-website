...
export const ADMIN_RANGES = ["7", "30", "90", "all"];
...
export function parseAdminFilters(input = {}) {
  ...
  return { page: Math.min(Math.max(parsedPage, 1), 100_000), search, status, source, range };
}
...
export function buildGrowthSeries(rows = [], startingActive = 0) {
  let runningActive = Math.max(0, number(startingActive));
  return rows.map((row) => {
    runningActive = row.active === null || row.active === undefined
      ? Math.max(0, runningActive + number(row.activations) - number(row.unsubscribes))
      : Math.max(0, number(row.active));
    return { day: String(row.day), signups: number(row.signups), active: runningActive };
  });
}
...
const OVERVIEW_QUERY = `WITH people AS (
  ...
  SELECT
    EXTRACT(EPOCH FROM date_trunc('day', NOW()))::bigint AS today_start,
    EXTRACT(EPOCH FROM date_trunc('day', NOW()) - INTERVAL '6 days')::bigint AS current_7_start,
    EXTRACT(EPOCH FROM date_trunc('day', NOW()) - INTERVAL '13 days')::bigint AS previous_7_start,
    EXTRACT(EPOCH FROM date_trunc('month', NOW()))::bigint AS month_start,
    EXTRACT(EPOCH FROM date_trunc('month', NOW()) - INTERVAL '1 month')::bigint AS previous_month_start,
    EXTRACT(EPOCH FROM date_trunc('month', NOW()) - INTERVAL '2 months')::bigint AS two_months_start,
    EXTRACT(EPOCH FROM date_trunc('day', NOW()) - INTERVAL '29 days')::bigint AS days_30_start,
    EXTRACT(EPOCH FROM date_trunc('day', NOW()) - INTERVAL '89 days')::bigint AS days_90_start
)
SELECT
  COUNT(*)::int AS total,
  ...
  (COUNT(*) FILTER (WHERE status IN ('active', 'unsubscribed') AND activated_at >= b.month_start)
    - COUNT(*) FILTER (WHERE unsubscribed_at >= b.month_start))::int AS net_this_month,
  (COUNT(*) FILTER (WHERE status IN ('active', 'unsubscribed') AND activated_at >= b.previous_month_start AND activated_at < b.month_start)
    - COUNT(*) FILTER (WHERE unsubscribed_at >= b.previous_month_start AND unsubscribed_at < b.month_start))::int AS net_previous_month,
  (COUNT(*) FILTER (WHERE status IN ('active', 'unsubscribed') AND activated_at >= b.days_30_start)
    - COUNT(*) FILTER (WHERE unsubscribed_at >= b.days_30_start))::int AS net_30_days,
  (COUNT(*) FILTER (WHERE status IN ('active', 'unsubscribed') AND activated_at >= b.days_90_start)
    - COUNT(*) FILTER (WHERE unsubscribed_at >= b.days_90_start))::int AS net_90_days,
  COUNT(*) FILTER (WHERE activated_at < b.month_start AND (status = 'active' OR (status = 'unsubscribed' AND unsubscribed_at >= b.month_start)))::int AS active_at_month_start,
...
function growthQuery(rangeDays) {
  return {
    text: `WITH limits AS (
      SELECT
        CASE WHEN $1::int IS NULL
          THEN COALESCE(date_trunc('day', to_timestamp(MIN(created_at))), date_trunc('day', NOW()))
          ELSE date_trunc('day', NOW()) - ($1::int - 1) * INTERVAL '1 day'
        END AS start_day,
        date_trunc('day', NOW()) AS end_day
      FROM subscribers
    ), days AS (
      SELECT generate_series(start_day, end_day, INTERVAL '1 day') AS day FROM limits
    ), people AS (
      SELECT status, created_at, unsubscribed_at, COALESCE(confirmed_at, created_at) AS activated_at FROM subscribers
    ), daily AS (
      SELECT days.day,
        COUNT(*) FILTER (WHERE date_trunc('day', to_timestamp(p.created_at)) = days.day)::int AS signups,
        COUNT(*) FILTER (WHERE p.status IN ('active', 'unsubscribed') AND date_trunc('day', to_timestamp(p.activated_at)) = days.day)::int AS activations,
        COUNT(*) FILTER (WHERE date_trunc('day', to_timestamp(p.unsubscribed_at)) = days.day)::int AS unsubscribes
      FROM days LEFT JOIN people p ON
        p.created_at < EXTRACT(EPOCH FROM days.day + INTERVAL '1 day')::bigint
        OR p.activated_at < EXTRACT(EPOCH FROM days.day + INTERVAL '1 day')::bigint
        OR p.unsubscribed_at < EXTRACT(EPOCH FROM days.day + INTERVAL '1 day')::bigint
      GROUP BY days.day
    ), baseline AS (
      SELECT COUNT(*)::int AS active_before
      FROM people, limits
      WHERE activated_at < EXTRACT(EPOCH FROM start_day)::bigint
        AND (status = 'active' OR (status = 'unsubscribed' AND unsubscribed_at >= EXTRACT(EPOCH FROM start_day)::bigint))
    )
    SELECT to_char(day, 'YYYY-MM-DD') AS day, signups,
      GREATEST(0, (SELECT active_before FROM baseline) + SUM(activations - unsubscribes) OVER (ORDER BY day))::int AS active
...
export async function loadAdminDashboard(query, filters) {
  ...
  return {
    ...
    delivery,
    sources,
    growth: buildGrowthSeries(growthRows),
    ...
    velocity: {
      weekly: comparePeriods(overview.current7, overview.previous7),
      monthly: comparePeriods(overview.thisMonth, overview.previousMonth)
    },
    subscribers,
    filteredCount,
    pageCount,
