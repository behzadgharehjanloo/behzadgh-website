...
const OVERVIEW_QUERY = `WITH people AS (
  ...
  SELECT
    COALESCE(SUM(CASE WHEN occurred_at >= b.month_start THEN CASE WHEN event_type = 'activated' THEN 1 ELSE -1 END ELSE 0 END), 0)::int AS net_this_month,
    COALESCE(SUM(CASE WHEN occurred_at >= b.previous_month_start AND occurred_at < b.month_start THEN CASE WHEN event_type = 'activated' THEN 1 ELSE -1 END ELSE 0 END), 0)::int AS net_previous_month,
    COALESCE(SUM(CASE WHEN occurred_at >= b.days_30_start THEN CASE WHEN event_type = 'activated' THEN 1 ELSE -1 END ELSE 0 END), 0)::int AS net_30_days,
    COALESCE(SUM(CASE WHEN occurred_at >= b.days_90_start THEN CASE WHEN event_type = 'activated' THEN 1 ELSE -1 END ELSE 0 END), 0)::int AS net_90_days,
    GREATEST(0, COALESCE(SUM(CASE WHEN occurred_at < b.month_start THEN CASE WHEN event_type = 'activated' THEN 1 ELSE -1 END ELSE 0 END), 0))::int AS active_at_month_start
  FROM subscriber_events CROSS JOIN boundaries b
)
SELECT
  COUNT(*)::int AS total,
  ...
  e.net_this_month,
  e.net_previous_month,
  e.net_30_days,
  e.net_90_days,
  e.active_at_month_start,
...
FROM people CROSS JOIN boundaries b CROSS JOIN event_summary e
GROUP BY e.net_this_month, e.net_previous_month, e.net_30_days, e.net_90_days, e.active_at_month_start`;
